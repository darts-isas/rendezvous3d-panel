...
**Key Parameters** カテゴリで、3D表示の上にデータフィールドの値を固定書式のテキストで重ねて表示します。枠のサイズは項目数・表示名・*Value Width* だけで決まり、値の内容によって変動することはありません。

- **Font Size**: オーバーレイの文字サイズ (px)。
- **Vertical Position** / **Horizontal Position**: 表示位置。`Top`/`Bottom` と `Left`/`Right` の組み合わせで4隅のいずれかに固定します。既定値: `Top` / `Left`。
- **Separator**: 表示名と値の間に置く区切り文字。`:` / `=` / 半角スペースから選択します。
- **Value Width**: 値の表示幅 (文字数)。この幅より短い値は空白で右詰めパディングされ、長い値は右側が切り詰められるため、値の桁数に関わらず幅は変わりません。
- **Text Color**: オーバーレイの文字色。
- **Background**: オーバーレイの背景を表示するかどうか。既定値: on、ニュートラルグレーで不透明度 `0.25`。
- **Background Color** / **Background Opacity**: *Background* が有効なときの背景色と不透明度 (`0`〜`1`)。
- **Shape**: 背景・枠の角の形状。`Rectangle` (四角) または `Rounded` (角丸)。
- **Border**: オーバーレイに枠線を表示するかどうか。既定値: on。
- **Border Color**: *Border* が有効なときの枠線の色。
- **Add Key Parameter**: オーバーレイに表示する値を追加します。下のリストに全項目が並び、クリックで選択、目のアイコンで表示/非表示、矢印で並び替え、ゴミ箱アイコンで削除します。

各キーパラメータは以下の設定を持ちます。

- **Name**: 表示名。名前列に表示されます。
- **Data Field**: 表示する値のフィールド。`Series.Field` 形式 (例: `A.mode`) でも、素のフィールド名 (`mode`) でも指定できます。オブジェクトの位置/クォータニオン用フィールドと異なり、数値だけでなく任意の型 (文字列・時刻・真偽値など) のフィールドを指定できます。
- **Format**: フィールドの最終値に適用する、`printf` 形式の指定子1個と任意のリテラル文字列 — 例: `%.2f`、`%.2f deg`、`%s`、`%d`、`%+.1e`。文字列中で最初に現れる指定子だけが値に置き換わり、それ以外はそのまま表示されます。フィールドが解決できない場合は値の代わりに `-` を表示します。

左上に配置した状態で Camera Settings の **Show Position and Distance** が有効な場合や、右上に配置した状態で **Save/Reset Camera** ボタンが表示されている場合は、それらと重ならないよう自動的に下へずれます。
