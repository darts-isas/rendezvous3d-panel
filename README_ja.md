...
- `Max Extrapolation [ms]`: 最新サンプル以後の動作上限です。`0`で外挿を無効化し、既定値は`5000` msです。

サンプルは検証・正規化して時刻順に保持されます。表示時刻を過去へ移動した場合や対象フィールドを変更した場合は、そのモデルの履歴をリセットします。終端が`now`の相対時間範囲は実時間に追従し、絶対時間範囲は指定された終端に固定されます。
