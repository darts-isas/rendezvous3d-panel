...
- `Opacity` を固定値またはフィールド連動で設定し、球体の見え具合を調整（詳細は後述の[Opacity](#opacity)を参照）
...
- `Opacity` を固定値またはフィールド連動で設定し、モデルの見え具合を調整（詳細は後述の[Opacity](#opacity)を参照）

#### Opacity

Sphere と 3D Model オブジェクトには `Opacity`（見え具合・不透明度）の設定があります（Polyline と Annotation は実体を持たないマーカーのため対象外です）。位置やクォータニオンと同様に、固定値（`Const`）とフィールド連動（`Field`）を切り替えられます。

- `Const`（既定値 `1`）: 固定値。`0`（完全に透明）〜`1`（完全に不透明）の範囲で指定します。
- `Field`: 数値フィールドの最新値をデータ更新のたびに反映します。`1` を超える値は完全不透明に、`0` を下回る値は完全透明に飽和します。

Opacity はモデル自体が持つ透過設定の上に乗算されるため、元々半透明なマテリアルを持つモデルは `Opacity` を `1` にしても完全に不透明にはなりません。
